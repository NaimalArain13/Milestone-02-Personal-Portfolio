...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      <nav className="hidden md:flex items-center gap-10">
        {[
          ...
          <Link
            key={href}
            href={href}
            className="text-sm font-semibold text-primary border-t-4 border-transparent hover:border-accent hover:text-accent transition-all duration-200 pt-2"
          >
            {label}
          </Link>
        ))}
      ...
      {/* Mobile menu */}
      {isOpen && (
        <nav className="absolute top-full left-0 right-0 bg-white border-t border-border shadow-sm flex flex-col items-center py-6 gap-6">
          {[
            { label: "Projects", href: "/projects" },
            { label: "Services", href: "/services" },
            { label: "Contact", href: "/contact" },
